...
<section class="bg-gray-100 py-30 px-4 sm:px-6 lg:px-8 font-family-montserrat">
  <div class="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-10">
    ...
    <div class="md:col-span-2 space-y-10">
      ...
      <div>
        ...
        <p class="text-sm text-gray-500 mt-1">Not ready to checkout? 
          <a href="/Assignment/Listing" class="underline hover:text-blue-600 transition-all duration-150 ease-in-out">
            Continue Shopping
          </a>
        </p>
        ...
        <div class="mt-6 border-b pb-6 space-y-6">
          <?php foreach ($cartItems as $items) : ?>
            <div class="flex flex-col sm:flex-row items-start gap-4">
              <img src="<?= $items['vehicle']['image'] ?>" alt="Car" class="w-full sm:w-60 object-cover rounded" />
              ...
              <a href="/Assignment/Customer/CartRemove?id=<?= $items['order']['id'] ?>" class="mt-2 sm:mt-0">
                <button class="text-sm text-gray-500 underline hover:text-red-400 transition-all duration-150 ease-in-out">
                  Remove
                </button>
              </a>
            </div>
    ...
    <aside class="space-y-6 md:col-span-1 w-full">
      <h3 class="text-lg font-semibold">Order Summary</h3>
      <div class="space-y-3 text-sm">
        <div class="flex justify-between">
          <span>Subtotal</span>
          <span>$<?= number_format($cartSummery['summary']['subtotal']) ?></span>
        </div>
        <div class="flex justify-between border-b pb-3">
          <span>Tax</span>
          <span class="text-gray-500">$<?= number_format($cartSummery['summary']['tax']) ?></span>
        </div>
        <div class="flex justify-between font-semibold">
          <span>Total</span>
          <span>$<?= number_format($cartSummery['summary']['total']) ?></span>
        </div>
      </div>
